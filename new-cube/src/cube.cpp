    ...
    for (int x = 0; x < 3; ++x) {
        for (int y = 0; y < 3; ++y) {
            for (int z = 0; z < 3; ++z) {
                ...
                // 如果在最左侧x=0位置，给左面贴纸
                if (x == 0) piece.faceColor[LEFT] = colLeft;
                // 最右侧x=2位置，右面贴纸
                if (x == 2) piece.faceColor[RIGHT] = colRight;
                // 最底层y=0位置，下表面贴纸
                if (y == 0) piece.faceColor[DOWN] = colDown;
                // 最顶层y=2位置，上表面贴纸
                if (y == 2) piece.faceColor[UP]   = colUp;
                // 最后排z=0位置，背面贴纸
                if (z == 0) piece.faceColor[BACK]  = colBack;
                // 最前排z=2位置，前面贴纸
                if (z == 2) piece.faceColor[FRONT] = colFront;
...
void CubePiece::rotateAroundX(bool clockwise) {
    ...
    if (clockwise) {
        // 顺时针 (以X轴正方向观察): Up->Front, Front->Down, Down->Back, Back->Up
        ...
        // 逆时针: Up->Back, Back->Down, Down->Front, Front->Up
...
void CubePiece::rotateAroundZ(bool clockwise) {
    ...
    memcpy(old, faceColor, sizeof(old));
    if (clockwise) {
        // 顺时针 (从Z轴正向即正前方看): Up->Left, Left->Down, Down->Right, Right->Up
        ...
        // 逆时针: Up->Right, Right->Down, Down->Left, Left->Up
